...
class DuoClient {
    ...
    async import_private_key(private_key) {
        ...
        const pemContents = private_key.trim().substring(pemHeader.length, private_key.length - pemFooter.length).replace(/\n/g,"");
    ...
    async import_public_key(public_key) {
        ...
        const pemContents = public_key.trim().substring(pemHeader.length, public_key.length - pemFooter.length).replace(/\n/g,"");
        // base64 decode the string to get the binary data
        const binaryDerString = window.atob(pemContents);
        // convert from a binary string to an ArrayBuffer
        const binaryDer = str2ab(binaryDerString);
    ...
    async import_key(keyPair) {
        ...
        this.keyPair = { privateKey: privateKey, publicKey: publicKey }
    }
    async generate_key() {
    ...
    async activate() {
        ...
        if (this.code) {
            ...
            if (resp["stat"] && resp["stat"] == "FAIL")
                throw resp["message"] || "Invalid activation code"
    ...
    async generate_signature(method, path, time, data) {
        ...
        return "Basic " + window.btoa(this.pkey + ":" + window.btoa(ab2str(signature)))
    }
    async get_transactions() {
        ...
        const resp = await response.json()
        if (resp["stat"] && resp["stat"] == "FAIL")
            throw resp["message"] ||  "Failed get_transactions()"
    ...
    async reply_transaction(transaction_id, answer) {
        ...
        const data = { "akey": this.akey,"answer":answer, "fips_status": "1", "hsm_status": "True", "pkpush": "rsa-sha512" }
        ...
        const resp = await response.json()
        if (resp["stat"] && resp["stat"] == "FAIL")
            throw resp["message"] || "Failed reply_transaction()"
        return resp["response"]
    }
}
